...
export function llmsIndex(pages, origin) {
  const lines = [
    ...
    '2. Every run makes five or seven agent calls and one or two Jev calls, all of',
    '   them billed. A run takes minutes. `--timeout` applies to each call',
    '   (default 600 seconds), not to the whole run.',
