...
const SUMMARY = [
  ...
  '`--mode fast` answers with the first draft Jev accepts on its own.',
  'The plan goes to stdout as Markdown, or as JSON with `--json`;',
  'progress goes to stderr. It never edits the repository. Also usable as a',
...
export function llmsIndex(pages, origin) {
  const lines = [
    ...
    '2. With the default two agents a run makes three to seven agent calls and one',
    '   to three Jev calls, all of them billed; `--mode ultra` makes at least five,',
    '   `--mode fast` as few as two.',
    ...
    '   mode, against the directory given by `--cwd`. Nothing is edited.',
