...
The new default, `--mode fast`, puts Jev's typed judgment in front of each round instead of after
...
- `--mode <fast|ultra>` chooses the pipeline; `mode` in `PlanOptions` does the same from code.
